...
export function AboutDialog({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  useEffect(() => {
    ...
    const observer = new MutationObserver(() => {
       // Just to ensure standard UI behaves smoothly
    });
  ...
  return (
    <div 
      className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm animate-in fade-in duration-200 p-4 app-region-no-drag"
      style={{
    ...
    >
      <div 
        className="bg-[var(--app-bg-panel)] border border-[var(--app-border-base)] rounded-lg shadow-xl w-full max-w-2xl flex flex-col overflow-hidden max-h-full animate-in zoom-in-95 duration-200"
      ...
      >
        <div className="flex items-center justify-between p-4 border-b border-[var(--app-border-base)] shrink-0 bg-[var(--app-bg-panel-alt)] rounded-t-lg">
          ...
          <button 
            onClick={onClose}
            className="text-[var(--app-text-muted)] hover:text-[var(--app-text-primary)] p-1 rounded-full hover:bg-[var(--app-bg-hover)] transition-colors"
          >
            <X size={20} />
          </button>
        </div>
          ...
          <div className="text-sm text-[var(--app-text-secondary)] leading-relaxed space-y-4">
            <p className="flex items-center gap-2 text-[var(--app-text-primary)] text-base">
              ...
              <a 
                ...
                target="_blank" 
                rel="noreferrer" 
                className="text-[var(--app-accent)] hover:underline inline-flex items-center gap-1"
              >
        ...
        <div className="p-4 border-t border-[var(--app-border-base)] flex justify-end shrink-0 bg-[var(--app-bg-panel-alt)] rounded-b-lg">
          <button 
            onClick={onClose}
            className="px-6 py-2 bg-[var(--app-bg-hover)] hover:bg-[var(--app-border-base)] text-[var(--app-text-primary)] rounded font-medium transition-colors"
          >
            關閉
          </button>
        </div>
